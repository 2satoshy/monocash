  ...
  return (
    ...
    >
      ...
      <div className="p-8 border-b border-zinc-100 flex justify-between items-end bg-white/80 backdrop-blur-md sticky top-0 z-10">
        <div>
          <h2 className="text-3xl font-black text-zinc-900 tracking-tight">Coordination</h2>
        </div>
        <div className="text-right">
          <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1">Team: {agent.department}</p>
              ...
              <div className={`flex items-start gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} max-w-[90%]`}>
                ...
                <div className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  <div className={`px-4 py-2.5 rounded-[20px] text-[14px] leading-relaxed shadow-sm ${
                    ...
                    {msg.text.includes(':') ? msg.text.split(':').slice(1).join(':').trim() : msg.text}
                  </div>
                  
                  <div className={`flex items-center gap-2 mt-2 px-1`}>
                    <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">
                      {msg.text.includes(':') ? msg.text.split(':')[0] : (msg.role === 'user' ? 'You' : agent.role.split(' ')[0])}
                    </span>
        ...
        <div className="relative flex items-center gap-2">
          <div className="flex-1 relative">
            <textarea
              ...
              placeholder="Coordinate strategy (↵ to send)"
