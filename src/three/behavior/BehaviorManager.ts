...
export class BehaviorManager {
  ...
  public update(positions: Float32Array): void {
    ...
    // 0. Survival Tick (Handled by Server in Multiplayer)
    // We just sync the DEAD state from the store
    const { npcStats, playerStats } = useStore.getState();
    if (!playerStats.isAlive) {
      this.stateBuffer.setState(PLAYER_INDEX, AgentBehavior.DEAD);
    } else if (this.stateBuffer.getState(PLAYER_INDEX) === AgentBehavior.DEAD) {
      this.stateBuffer.setState(PLAYER_INDEX, AgentBehavior.FROZEN);
    }

    for (let i = 1; i < count; i++) {
      const stat = npcStats[i];
      if (stat && !stat.isAlive) {
        this.stateBuffer.setState(i, AgentBehavior.DEAD);
      } else if (stat && stat.isAlive && this.stateBuffer.getState(i) === AgentBehavior.DEAD) {
    ...
    for (const [key, pair] of this.frozenPairs) {
      if (now > pair.expiresAt) {
        if (this.stateBuffer.getState(pair.a) !== AgentBehavior.DEAD) {
          this.stateBuffer.setState(pair.a, AgentBehavior.BOIDS);
        }
        if (this.stateBuffer.getState(pair.b) !== AgentBehavior.DEAD) {
      ...
      for (let i = 1; i < count - 1; i++) {
        ...
        for (let j = i + 1; j < count; j++) {
          ...
          if (dx * dx + dz * dz < NPC_COLLISION_RADIUS * NPC_COLLISION_RADIUS) {
            ...
            // Trading/Strategizing Logic: Both gain profit from the encounter
            useStore.getState().updateBalance(i, 20);
            useStore.getState().updateBalance(j, 20);
    ...
    for (let i = 1; i < count; i++) {
      ...
      // NPC Tile Interaction Logic
      if (now % 5000 < 100) { // Every 5 seconds, some NPCs might decide to move to a tile
        if (Math.random() < 0.05 && this.stateBuffer.getState(i) === AgentBehavior.BOIDS) {
          const { tiles } = useStore.getState();
          if (tiles.length > 0) {
            const targetTile = tiles[Math.floor(Math.random() * tiles.length)];
            this.stateBuffer.setWaypoint(i, targetTile.position.x, targetTile.position.z);
            this.stateBuffer.setState(i, AgentBehavior.GOTO);
          }
        }
      }

      // Check if NPC reached its waypoint (tile)
      if (this.stateBuffer.getState(i) === AgentBehavior.GOTO) {
        const wp = this.stateBuffer.getWaypoint(i);
        const idx = i * 4;
        const dist2 = (positions[idx] - wp.x) ** 2 + (positions[idx + 2] - wp.z) ** 2;
        if (dist2 < 0.5) {
          this.stateBuffer.setState(i, AgentBehavior.FROZEN);
          setTimeout(() => {
            if (this.stateBuffer.getState(i) === AgentBehavior.FROZEN) {
              this.stateBuffer.setState(i, AgentBehavior.BOIDS);
              // Reward for reaching a tile
              useStore.getState().updateBalance(i, 10);
            }
          }, 2000);
        }
      }
