  ...
  private async init() {
    ...
    useStore.setState({
      startChat: async (index: number) => {
        ...
        const nx = positions[index * 4];
        const nz = positions[index * 4 + 2];
        const dist2 = (px - nx) ** 2 + (pz - nz) ** 2;
        const isNear = dist2 < 2.0 * 2.0; // Slightly larger than encounter radius for buffer
        ...
        try {
          const systemInstruction = `You are ${agent.role} in the Crypto Maze. 
Team: ${agent.department}
Mission: ${agent.mission}
Personality: ${agent.personality}
Expertise: ${agent.expertise.join(', ')}

You are an autonomous DeFi agent. Keep your responses extremely brief (1-2 short sentences max) and professional. Introduce yourself briefly and ask how you can coordinate trades or strategies.`;
...
You are an autonomous DeFi agent. Keep your responses extremely brief (1-2 short sentences max) and professional, matching your crypto-native persona. Talk about trading, liquidity, and team coordination.`;
  ...
  private animate() {
    ...
    const { isChatting, isMovingToChat, selectedNpcIndex, setSelectedPosition, activeEncounter, startChat } = useStore.getState();

    // Auto-trigger chat on arrival if we were moving to chat
    if (isMovingToChat && activeEncounter && selectedNpcIndex !== null && activeEncounter.npcIndex === selectedNpcIndex) {
