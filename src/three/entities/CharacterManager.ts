...
export class CharacterManager {
  ...
  private initComputeNode() {
    ...
    this.computeNode = Fn(() => {
      ...
      const agentState = agentData.w;                   // float: 0=BOIDS 1=FROZEN 2=GOTO 3=DEAD

      const pos = posElement.xyz.toVar();

      // ── DEAD (state ≈ 3, > 2.5) ───────────────────────────
      If(agentState.greaterThan(float(2.5)), () => {
        velElement.assign(vec4(0.0));
        posElement.assign(vec4(pos, 1.0));
      }).Else(() => {
        // ── NOT DEAD ─────────────────────────────────────────
        If(agentState.greaterThan(float(0.5)), () => {
          // ── FROZEN or GOTO (state > 0.5) ────────────────────
  ...
  private createVertexNode() {
    return Fn(() => {
      ...
      // Handle DEAD state (scale to 0)
      const agentStateForScale = this.agentStateBuffer!.storageNode.element(instanceIndex).w;
      const isDead = agentStateForScale.greaterThan(float(2.5));
      finalPosition.mulAssign(isDead.select(float(0), float(1)));
